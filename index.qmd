...
También se realizaron pruebas de varianza y varianza acumulada para identificar cuantos componentes son necesarios para explicar de forma significativa la varianza. Esto para reducir la cantidad de PCA y no perder demasiada información. Primero realizamos una prueba de varianza y luego de la acumulada. En la varianza acumulada identificamos que con 3 PC explicamos más del 90% de la varianza, siendo la mayor parte de información. Por lo que el último componente principal solo aporta un 9% de varianza.
...
En los siguientes dos gráficos se muestra primero la varianza acumulada según los PCA, donde con 3 PCA ya tenemos el 90% de la varianza. Mientras que en el otro gráfico se observa la distribución regional del país en cuanto a variables de consumo de alcohol.

Acá se puede observar que regiones como Bío-Bío tiene mayor probabilidad de consumo de alcohol que la Araucanía, y además mayor probabilidad de encontrar personas que manejan habiendo ingerido alcohol. No así en regiones como Arica y Tarapacá, Valparaíso y la Metropolitana.
...
Obtenidos los PCA de alcohol, también realizamos un análisis de componentes principales para nuestra variable de Marihuana siguiendo los mismos pasos de estandarización y pruebas de varianza.

Esta selección cumple el mismo objetivo de identificar las principales variables que explican la varianza de los datos para generar clusters. Se utilizaron las mismas variables de Alcohol pero especificas de Marihuana. Siendo mar_rec la prevelancia de consumo (declarado) de marihuana, la percepción de riesgo sobre el consumo de marihuana, y si ha  manejado habiendo consumido de la marihuana.
...
Finalmente, las pruebas de varianza y varianza acumulada de los primeros 3 componentes principales indican que estos explicarían casi un 90% de la varianza, mientras que con los otros solo se explica el 10% restante. Esto es importante ya que perdemos poca información con el menor número de grupos posibles.
...
Para una mejor visualización de la varianza de nuestras variables en las regiones de Chile, primero se muestra el gráfico de varianza acumulada y, en segundo lugar, su distribución en las regiones. En base esto identificamos que:

Como mencionamos en el consumo y regiones, Los Lagos es una región cercana a nuestra variable mar_rec, indicando que el consumo de marihuana es predominante en comparación a otras regiones del país. Se trata de casi la única región relativamente cercana tanto al consumo declarado de maihuana como al manejo habiendo fumado marihuana. 


Con respecto a la percepción de riesgo de fumar marihuana frecuentemente y el riesgo en uso experimental, Arica y Parinacota, Tarapacá y Atacama se encuentran cercanas a estas variables, por lo que es probable que las personas consideren que es riesgoso. No así en regiones como Coquimbo o Bío-Bío donde si bien no es predominante el consumo de marihuana no consideran riesgosos esos habitos.
...
Usaremos un K-means clustering para identificar los posibles grupos en la base. Primero ejectuamos una semilla para asegurar que se obtengan los mismos resultados debido a la aleatoriedad de los centroides (los puntos que representan cada cluster en el algoritmo). Luego implementamos el método wss para identificar el número óptimo de clusters para el análisis de los k-means. En el gráfico sobre el método se observa que al tener 1 o 2 cluster la suma de los errores cuadrados es alta lo que no asegura una variabilidad en los datos. No obstante, a medida que se suman más clusters los datos se van agrupando y explican el modelo. En base al gráfico optamos por 4 cluster para el análisis.

Con los resultados del k-means identificamos que las regiones fueron agrupadas en 4 grupos en base a las características de las variables seleccionadas sobre consumo, percepción de riesgo y manejo habiendo consumido alcohol previamente. Estos grupos se distribuye de la siguiente manera:

\- Grupo 1: Metropolitana, Tarapacá, Valparaíso, Araucanía y Ñuble.

\- Grupo 2: Arica y Parinacota, Coquimbo, Metropolitana, Maule, Los Ríos, Aysén y Magallanes.

\- Grupo 3: Antofagasta, Atacama y Maule.

\- Grupo 4: O'Higgins, Bíobío, Los Lagos.

En cuanto a los resultados promedio de cada cluster podemos inferir lo siguiente:

\- Grupo 1: Las regiones presentan un menor consumo de alcohol en comparación con los otros clusters, tienen una alta percepción de riesgo y es poco probable que manejen habiendo consumido alcohol previamente.

\- Grupo 2: Las regiones presentan un consumo moderado de alcohol, baja percepción de riesgo al beber de forma ocasional pero sí al beber frecuentemente, y poco probable que manejen habiendo consumido alcohol previamente.

\- Grupo 3: Regiones con alto consumo de alcohol, percepción de riesgo moderada respecto al consumo ocasional pero alta a beber frecuentemente y poco probable que manejen habiendo consumido alcohol previamente

\- Grupo 4: Regiones con alto consumo de alcohol, baja percepción de riesgo respecto al consumo ocasional de alcohol pero también con respecto al consumo frecuente de alcohol. Con una probabilidad moderada de manejo habiendo consumido alcohol previamente. 
...
\- Grupo 1:  Atacama y Araucanía

\- Grupo 2: Ñuble, Tarapacá y Antofagasta

\- Grupo 3: Arica y Parinacota, Metropolitana, Los Ríos, Magallanes, Vapariso Maule y Aysén

\- Grupo 4: Coquimbo, O'Higgins, Biobío y Los Lagos

En cuanto al resultado promedio por grupo se observa lo siguiente:

\- Grupo 1: Región con personas con consumo bajo de marihuana, percepción moderada de riego en el consumo experimental y alta en el consumo frecuente. Poco probable conducir habiendo consumido la sustancia. Sobre opinión pública, están más de acuerdo con permitir la marihuana de uso terapeutico y con la premisa de que la marihuana hace menos daño que el alcohol.

\- Grupo 2: Las regiones del cluster 2 presentan un consumo moderado de marihuana, leve percepción de riesgo en el uso de marihuana de forma experimental pero una percepción de riesgo más alta al consumir marihuana de forma frecuente. Es poco probable que manejen después de consumir marihuana. En cuanto opinión pública están menos de acuerdo con el uso de forma terapeutica y están más de acuerdo con que hace menos daño alcohol.

\- Grupo 3: Regiones con el menor consumo de los tres clusters, y mayores niveles de percepción de riesgo sobre el consumo tanto experimental como frecuente. También menor probabilidad de manejar habiendo consumido marihuana. Sobre opinión pública es el grupo que está más de acuerdo con el uso de marihuana de forma experimental pero presenta niveles moderados en cuanto que hace menos daño que el alcohol.
...
Grupo 3: Regiones con mayor probabilidad de consumo, percepción moderada del riesgo que represeta el consumo de alcohol ocasional pero alta cuando ese consumo se convierte en frecuente. También con una baja ocurrencia de manejo habiendo consumido alcohol previamente.
Grupo 4:  Presenta un perfil distinto, con mayor consumo, baja percepción de riesgo sobre el consumo ocasional y el consumo frecuente, y una ligeramente mayor probabilidad de manejar habiendo consumido alcohol previamente.
...
Grupo 1: Consumo moderado, percepción moderada del riesgo del sonsumo experimental de marihuana y percepción alta del reisgo que supoen un consumo frecuente. Apoyo al uso terapéutico de marihuana y acuerdo con que causa menos daño que el alcohol.
