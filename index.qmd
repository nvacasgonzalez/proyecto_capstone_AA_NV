...
paquetes <- c("dplyr", "haven", "rio", "ggplot2", "psych", "tidyr", "table1", "knitr", "kableExtra", "survey", "plotly", "sf", "tmap")
...
```{r, echo=TRUE, results="hide"}
# Tuvimos algunos problemas para generar el mapa interactivo. Básicamente, porque nuestro shape file tenía geometrías invalidas  geometrías son válidas
invalid_geometries <- chile_map[!st_is_valid(chile_map), ]
print(invalid_geometries)

```

```{r, echo=TRUE, results="hide"}
# Verificar si hay geometrías no válidas
invalid_geometries <- chile_map[!st_is_valid(chile_map), ]
print(invalid_geometries)
...
chile_map <- st_make_valid(chile_map)

```

```{r, echo=TRUE, results="hide"}
# Verificar que ahora todas las geometrías sean válidas
st_is_valid(chile_map)  # Debería devolver TRUE para todas las geometrías
```
```{r}
# Activamos el modo interactivo de tmap
tmap_mode("view")

# Generamos el mapa interactivo con la paleta de verdes
tm_shape(chile_map) +
  tm_polygons("Marihuana (%)",   # Aseguramos que mostramos el porcentaje de marihuana en la capa
              palette = "BuGn",  # Paleta de colores suaves de verde (azul a verde)
              ...
              popup.vars = c("Region", "Marihuana (%)"),  # Mostramos tanto el nombre de la región como el porcentaje de marihuana en el popup
              showNA = FALSE) +  # Evita mostrar valores NA si existen
...
# Generamos el mapa interactivo con la paleta de colores amarillos a rojizos
tm_shape(chile_map) +
  tm_polygons("Alcohol (%)",   # Aseguramos que mostramos el porcentaje de alcohol en la capa
              palette = "YlOrRd",  # Paleta de colores de amarillo a rojo
              title = "Porcentaje de Consumo de Alcohol (%) por Región",  # Título de la leyenda
              style = "jenks",  # Estilo de clasificación para el color
              popup.vars = c("Region", "Alcohol (%)"),  # Mostramos tanto el nombre de la región como el porcentaje de alcohol en el popup
              showNA = FALSE) +  # Evita mostrar valores NA si existen
  tm_layout(legend.title.size = 1.4, 
            frame = FALSE)  # Personalizar tamaño de título de la leyenda y quitar borde
