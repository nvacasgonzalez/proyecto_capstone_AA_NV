...
# Otras variables interés de para la aplicación de modelos de Machine Learning no supervisados

Para la generación de posibles clusters fueron seleccionadas, adicionalmente, un conjunto de variables relativas a la percepción de riesgo (PR), conducción con consumo declarado de alcohol o marihuana (CC_2 y CC_3, repectivamente) y opinión pública (OP). En relación a estas variables fueron seleccionadas solo las categorías que hacen referencia sea a alcohol o marihuana, ya que existen otras categorías sobre otros tipos de droga.
...
Dentro de la Encuesta Nacional hay un apartado sobre actividades que han realizado bajo el consumo de diferentes tipos de drogas. En línea con nuestra investigación seleccionamos dos preguntas de caractér binaria sobre si ha consumido bajo el efecto del alcohol o de marihuana. Esto de forma exploratoria y debido al bajo porcentaje que afirma haber conducido bajo el efecto de alguna droga, mostramos el número de observaciones.

En cuanto a personas que han manejado después de beber alcohol independiente del número de tragos, un 3,9% afirma haberlo hecho. Mientras que un 1,2% afirma haber manejado después de consumir marihuana.
...
En los siguientes dos gráficos se muestra primero la varianza acumulada según los PCA, donde con 3 PCA ya tenemos el 90% de la varianza. Minetras que en el otro gráfico se observa la distribución regional del país en cuanto variables de consumo de alcohol.

Acá se puede observar que regiones como Bío-Bío tiene mayor probabilidad de consumo de alcohol que la Araucanía, y además mayor probabilidad de encontrar personas con que manejan bajo el efecto del alcohol. No así en regiones como Arica y Tarapacá, Valparaíso y la Metropolitana.

En cuanto las variables de percepción de riesgo sobre tomar alcohol de manera ocasional y frecuentemente, regiones como Aysén y Atacama se encuentran cercanas al considerar ambos hábitos como riesgosos, a diferencia de regiones como Maule, O'higgins e incluso Los Lagos que se encuentra más alejados.
...
Esta selección cumple los mismos objetivos de identificar las principales variables que explican la varianza de los datos para generar clusters. Se utilizaron las mismas variables de Alcohol pero especificadas en Marihuana. Siendo mar_rec la prevelancia de consumo de marihuana, percepción de riesgo sobre el consumo de marihuana, si ha conducido después de manejar bajos efectos de la marihuana.

En cuanto a los PCA se generaron 6 componentes principales. Esto debido a la mayor cantidad de variables a comparación con el modulo de Alcohol. Nuestra variable mar_rec tiene un valor positivo de correlación en PC1, siendo el componente principal que puede explicar de mejor forma el consumo de marihuana.

En línea las pruebas de varianza y varianza acumulada con los primeros 3 componentes principales se explica casi un 90% de la varianza, mientras que con los otros solo se explica el 10% restante. Esto es importante ya que perdemos poca información con menor grupos posibles.
...
senda_clusters_mar <- senda %>%
  select(region, 
         ...
         maneja = cc_3, 
         permitir_marihuana_exp = op_2_3_rp, 
...
\- Grupo 1: Arica y Parinocata, Metropolitana, Maule, Ñuble, Los Ríos y Aysén
\- Grupo 2: Antofagasta, Valparaíso y Magallanes
...
cluster_means_mar %>%
  rename(
    ...
    "Manejo bajo efectos" = "maneja",
     "Acuerdo sobre Marihuana experimental" = "permitir_marihuana_exp",
